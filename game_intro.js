...
function draw_line(src_x, src_y, dst_x, dst_y, color, width) {
...
var brick_w, brick_h, brick_x, brick_y;
var speed_x, speed_y;
var acclr_x, acclr_y;
...
function draw_brick() {
	/*
	   ctx.fillStyle = "#FFF";
	   ctx.fillRect(brick_x, brick_y, brick_w, brick_h);
	   */
	ctx.save();
	ctx.translate(brick_x + brick_w / 2, brick_y + brick_h / 2);
	ctx.rotate(Math.PI / 4 * speed_y / 30);
	ctx.beginPath();
	ctx.rect(- brick_w / 2, -brick_h / 2, brick_w, brick_h);
	ctx.styoleStyle = "#FFF";
	ctx.stroke();
	ctx.restore();
}
...
var base_line_h;
function draw_base_line() {
	draw_line(0, base_line_h, canvas_w, base_line_h);
}
...
function update() {
	...
	if (speed_y < base_line_h - brick_h) {
		speed_y += acclr_y;
		if (brick_y >= base_line_h - brick_h) {
			speed_y = 0;
		}
	}		
	brick_x = brick_x > canvas_w ? 0 : brick_x;
}
...
function game_intro() {
	/*
	   ...
	   game_intro_anima();
	   */
	base_line_h = canvas_h / 5 * 4;

	brick_w = brick_h = 20;
	brick_x = brick_w;
	brick_y = base_line_h - brick_h;
	speed_x = 0;
	acclr_y = 5;
	speed_y = 0;
